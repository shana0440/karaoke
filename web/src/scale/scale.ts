...
export function makeScale(domain: [number, number], range: [number, number]): Scale {
    return {
        domain: domain,
        viewDomain: domain,
        range: range
...
export function mapRangeToViewDomain(scale: Scale, range: [number, number]): [number, number] {
    const domainDelta = scale.viewDomain[1] - scale.viewDomain[0];
    const rangeDelta = scale.range[1] - scale.range[0];
    const scaleRatio = domainDelta / rangeDelta;

    const viewDomain = range.map((value) => (value - scale.range[0]) * scaleRatio + scale.viewDomain[0]);

    return viewDomain as [number, number];
}

export function mapViewDomainToRange(scale: Scale, domain: [number, number]): [number, number] {
    const domainDelta = scale.viewDomain[1] - scale.viewDomain[0];
    const rangeDelta = scale.range[1] - scale.range[0];
    const scaleRatio = rangeDelta / domainDelta;

    const range = domain.map((value) => (value - scale.viewDomain[0]) * scaleRatio + scale.range[0]);
